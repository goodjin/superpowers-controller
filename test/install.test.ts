...
`)
  ...
  test("one-click install script installs idempotently through the local CLI", () => {
    ...
    writeFileSync(join(home, ".config", "opencode", "package.json"), JSON.stringify({ dependencies: { "@opencode-ai/plugin": "1.3.10", "@mem9/opencode": "file:///tmp/mem9", "oh-my-openagent": "latest" } }, null, 2))
    writeFileSync(join(home, ".config", "opencode", "package-lock.json"), "{}\n")
    writeFileSync(join(home, ".config", "opencode", "oh-my-openagent.json"), "{}\n")
    writeFileSync(join(home, ".opencode", "package.json"), JSON.stringify({ dependencies: { "@opencode-ai/plugin": "1.4.0", "opencode-superpowers-controller": "latest" } }, null, 2))
    ...
    expect(userPackage.dependencies["oh-my-openagent"]).toBeUndefined()
    ...
    expect(existsSync(join(home, ".config", "opencode", "node_modules", "oh-my-openagent"))).toBe(false)
    ...
    expect(homePackage.dependencies["opencode-superpowers-controller"]).toBeUndefined()
    ...
    expect(existsSync(join(home, ".opencode", "node_modules", "opencode-superpowers-controller"))).toBe(false)
