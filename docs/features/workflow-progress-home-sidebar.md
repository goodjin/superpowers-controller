...
问题出在 TUI 常驻进度面：`sidebar_content` 之前只在 slot 带 session props 时显示，部分全局 sidebar 场景会返回空；主会话 prompt 附近也没有注册 session 相关短进度 slot。结果是界面能看到 workflow 处于 running，但用户看不到节点进度、当前 phase 或最近 activity。

## 目标

- `sidebar_content` 在没有 session props 的首页/全局场景也展示当前 workflow 进度。
- `session_prompt_right` 展示一条短 compact 进度，作为主会话可见锚点。
...
- 扩展 resident slot 列表，注册 `session_prompt_right`。
...
- `session_prompt_right` 保持短文本，不承载完整明细。

## 验证

- 单元测试覆盖：
  - `sidebar_content` 无 session props 时仍显示 workflow 和运行节点。
  - `session_prompt_right` 已注册，并输出 compact progress。
