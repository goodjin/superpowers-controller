...
- Keep release validation in the workflow: install, CI-safe tests, build, package dry run, duplicate-version check, publish.
...
3. Run `bun install --frozen-lockfile`, CI-safe Bun tests, `bun run build`, and `npm pack --dry-run` before publish.
...
- Local release verification remains responsible for `deploy-superagent-runtime.test.ts`, which exercises the isolated OpenCode runtime launcher.
