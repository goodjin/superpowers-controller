...
Superpowers 的核心不是某条固定流程，而是一组工作纪律。controller 应把这些纪律用于澄清任务、准备任务文档、判断 designer 是否参与，并在 `sp_start` 中选择内置 workflow 代号或自定义编排，而不是把用户需求硬套到 `feature/debug/plan-only` 等固定流程。
...
- 用户明确要求“只调查”“先设计”“不要改代码”“不跑测试”等约束时，prepared task 和 start plan 必须体现这些限制。
...
- 方案不清、边界不清、架构取舍不清时，controller 在 start plan 中安排 `sp-designer`。
- 多步骤实现、跨模块修改、需要任务依赖时，controller 在 start plan 中安排 `sp-planner` 或选择包含 planner 的内置 workflow。
- 每个插件执行任务都先经过 `sp_prepare`，由插件生成任务文档和用户确认摘要。
- start plan 可以是内置 workflow 代号，也可以是自定义 orchestration；自定义 orchestration 可以只有一个节点。
- plan 后的执行任务由 planner 或后续 agent 的 `sp_report` 产出，并在 `allow_auto_expansion=true` 时由插件校验后自动扩展。
...
controller 设计 start plan 时应从任务性质选择这些纪律。例如 bugfix 工作不应直接进入 implementer，而应先安排 debugger 或调查节点；完成交付前不应只依赖 implementer 的口头总结，而应安排 verifier 或等价验证节点。
...
- controller 可以调用 `sp_prepare` 准备任务，并在用户确认后调用 `sp_start` 提交 start plan，但不直接创建 child session。
- child session 只能由插件在 `sp_start` 或 `sp_report` 触发的 transition 中创建或恢复。
- node agent 只能执行当前 node，并通过 `sp_report` 汇报结果。

### 2.5 插件是运行时事实源

模型可以推理和规划，但 prepared task、start plan、节点运行、恢复、取消、fallback、progress 都应由插件持久化并反馈。插件不具备需求理解智能，不能替 controller 选择 designer、agent 或业务 workflow；它只能暴露 agent catalog、workflow schema、built-in workflow templates、常用 workflow 示例和校验结果。

设计原则：

- `sp_status` 是 controller 对齐事实的入口。
- prepared task、start plan、node state、`sp_report`、fallback summary 是 transition 的依据。
...
- 需要代码变更的工作，start plan 应包含测试、构建、审查或验证节点，除非用户明确裁剪。
...
Every task that will be executed by the plugin must go through sp_prepare first. sp_prepare is for task preparation, durable task documents, and final user confirmation. It does not dispatch agents and it does not decide whether designer is needed.

After the user confirms the prepared task, call sp_start. sp_start receives the actual start plan. The start plan may reference a built-in workflow id, or provide a custom workflow orchestration. A custom orchestration may contain only one node. Decide whether sp-designer participates by including or excluding a designer node in the start plan.

Plan the start plan from the nature of the task, not from fixed workflow names. Use Superpowers skills as process disciplines:
...
- Design-only or plan-only: design or plan node -> terminal, with allow_auto_expansion=false.
...
Set allow_auto_expansion in sp_start. Use true when planner or agent reports may append implementation/check/finish nodes. Use false for design-only, plan-only, review-only, or bounded single-node work.

Do not execute delegated node work yourself. Do not edit files, load business skills, or call the native task tool. Use sp_status to read runtime facts and deterministic capabilities, sp_prepare to prepare and confirm tasks, sp_start to activate/resume/retry with a start plan, sp_cancel to stop, and rely on node agents to call sp_report.
...
- 输入 request、task brief 和 confirmation policy。
- 校验 task brief 是否包含 goal、scope、constraints 和 acceptance criteria。
- 写入 prepared task state。
- 写入 `prepared-task.json`、`request.md`、`task.md`、`documents.json` 和 `events.jsonl`。
...
- 插件把 prepared task 和确认摘要返回给 controller。
- controller 在主会话把 `confirmation_summary` 展示给用户。
- 用户要求修改时，controller 重新澄清后再次调用 `sp_prepare`。
- 用户确认后，controller 调用 `sp_start` 并提交 start plan。

### 4.3 `sp_start`

定位：基于 prepared task 和 start plan 激活、恢复、重试、继续执行。

调用场景：

- 用户确认 prepared task 后启动。
...
- 根据 `allow_auto_expansion` 设置后续 report 是否能扩展 workflow。
...
- 如果存在已校验的 report-driven expansion，且 `allow_auto_expansion=true`，插件应用 expansion 并派发新的 runnable node，不要求 controller 重新规划。
- 如果 `allow_auto_expansion=false`，插件不应用 report 中的新任务，只保存 artifact，并按 start plan 的 completion policy 结束或继续已有节点。
...
- terminal report 进入统一 transition，插件根据 prepared task、start plan edge、completion_policy 和 fallback_policy 计算下一步。
- report 不能包含 `next_action`、`next_suggestion`、`child_session_id`、`reuse_session_id` 或其它 control-plane 字段。node agent 的观察只能放进 `summary` 或 `findings`，由 controller 或 start plan 决定是否采用。
...
- 用户要求停止、取消、放弃当前工作。
- controller 判断 prepared task 或 start plan 错误、风险过高、状态混乱且不宜继续。
...
-> controller calls sp_prepare with clarified task brief
-> plugin writes prepared task documents and confirmation summary
-> controller asks user to confirm the prepared task
-> user confirms
-> controller calls sp_start with built-in workflow id or custom orchestration
```

这个循环中，大模型负责理解和决策，包括判断 designer 是否参与、选择内置 workflow 或自定义编排、解释反馈。插件负责暴露能力目录、生成任务文档、校验 start plan、落盘和派发。controller 不直接创建 session。
...
- `task_graph`：插件只做确定性转换。每个 task 变成一个 node，agent 来自 task.agent 或 start plan auto expansion policy 的 `default_task_agent`，depends_on 变成 edges，`default_check_chain` 变成附加检查节点。缺少 agent、超出 allowed_target_agents、超出数量或深度限制时，不猜测补齐。
- `allow_auto_expansion=false`：插件保存 report 和 artifact，但不把新增任务加入 workflow。该模式用于只做设计、只做计划、只跑指定节点等场景。
...
- `sidebar_content` 显示 prepared task、start plan、node graph、reports、fallback 和 attention。
...
v5 需要把插件可控的 workflow artifacts 作为 prepared task 和 start plan 的显式 contract，而不是让节点随手查找文件。

- runtime control documents: 插件生成，用于调度、恢复和审计，例如 `prepared-task.json`、`start-plan.json`、兼容性的 `workflow-spec.json`、`documents.json`、`nodes/<node-id>/task.md`、`nodes/<node-id>/record.json`、`nodes/<node-id>/fallback-summary.json`。
...
### 6.1 start plan 中的文档协议

controller 提交 start plan 时，应声明文档 contract：
...
- `sp_prepare`: 插件写入 `prepared-task.json`、`request.md`、`task.md`、`documents.json`、prepared state 和 `events.jsonl`。
- `sp_start(start_prepared_task)`: 插件写入 `start-plan.json`，并把内置 workflow 或自定义 orchestration 规范化为兼容性的 `workflow-spec.json`。
...
- report-driven expansion：如果 `sp_report` 同时包含 task graph 或 workflow expansion，插件在 canonical artifact 生成后按 start plan 的 auto expansion policy 校验和追加节点。
...
- `sp-designer` 产出的 spec 通常在 `passed` 后成为 canonical `spec.md`，由插件传给 planner；如果 start plan 明确要求人工确认，可设为 `on_controller_approval`。
- `sp-planner` 产出的 plan/task graph 通常在 `passed` 后成为 canonical `plan.md`、`task_graph.json` 和 `tasks.json`；当 `allow_auto_expansion=true` 时，插件据此追加 implementer、reviewer、verifier 等后续节点。
...
| 用户要求修改 prepared task | 修改 task brief 后重新 prepare | `sp_prepare` |
| 用户确认 prepared task | 选择内置 workflow 代号或自定义编排并启动 | `sp_start(action="start_prepared_task")` |
| 需要 designer 参与 | 在 start plan 中加入 designer node，或选择包含 designer 的内置 workflow | `sp_start` |
| 不需要 designer 参与 | start plan 只包含后续执行节点；可只有一个 node | `sp_start` |
| 只做 design/plan/review | 设置 `allow_auto_expansion=false` | `sp_start` |
...
| node report 产生 expansion 但 `allow_auto_expansion=false` | 插件保存 artifact，不追加节点；按 start plan completion policy 结束或继续 | `sp_status` 按需 |
| node report 产生 expansion 但校验失败 | controller 查看错误后 revise workflow、retry 或 cancel | `sp_status` -> `sp_prepare` / `sp_start` / `sp_cancel` |
| node report failed/blocked 且 start plan 无明确 edge | controller 选择 retry/revise/cancel | `sp_status` -> `sp_start` 或 `sp_cancel` |
...
- `super-agent` prompt 需要加入 prepare-first 工具协议、designer 参与判断、built-in workflow templates 和常用 workflow 示例，但明确 templates/examples 不是插件智能建议。
- `sp_prepare` 当前仍围绕固定 kind / prepare mode，需要改为 task preparation、task document persistence 和 confirmation summary。
- `sp_start` 需要支持 `built_in_workflow` 和 `orchestration` 两类 start plan，并允许 orchestration 只有一个 node。
- transition 仍主要由固定 router/workflow 规则驱动，需要改为读取 prepared task、start plan 和 workflow state。
- 需要实现 `allow_auto_expansion`，以及 `sp_report.task_graph` / `sp_report.workflow_expansion` 到 workflow graph 的自动追加。
- 需要给自动扩展增加最大节点数、允许来源 node、允许目标 agent、默认 task agent、递归深度和默认 check chain 等 guard。
- 需要持久化 `prepared-task.json`、`start-plan.json`、兼容性的 `workflow-spec.json` 和 `documents.json`，并把静态能力目录、built-in templates、常用示例和 run state 分开。
...
- 不把 Superpowers 理念固化为固定 workflow；controller 根据任务性质决定 start plan。
...
2. 扩展 `sp_prepare` / state schema，支持 prepared task、task document persistence、confirmation summary 和 `documents` 注册。
3. 扩展 `sp_start` / transition，支持 built-in workflow id、自定义 orchestration、单节点 orchestration 和 `allow_auto_expansion`。
4. 改 document promotion 与 fallback runtime，使 start plan、文档 contract、`sp_report` 和 fallback summary 进入统一调度入口。
