...
模型可以推理和建议，但 workflow 的真实状态、节点运行、恢复、取消、fallback、progress 都应由插件持久化并反馈。
...
Generate a workflow_spec when starting new work. Each node must have an agent, a scoped task, required context, expected output, report_contract, and transition condition. Ask the user to confirm workflow execution when the workflow changes files, runs high-risk commands, or the project instructions require confirmation.

Do not execute node work yourself. Do not edit files, load business skills, or call the native task tool. Use sp_status to read runtime facts, sp_prepare to request suggestions or register a confirmed workflow_spec, sp_start to activate/resume/retry, sp_cancel to stop, and rely on node agents to call sp_report.
...
- v5 中还应返回 agent catalog / workflow suggestion catalog，但只能作为建议。
...
定位：让插件给建议，或注册 controller 已生成并确认的 workflow spec。
...
- controller 已理解用户需求，但希望参考插件的 agent catalog 和建议节点。
- controller 不确定是否需要 designer/planner/debugger/verifier。
- 用户要求“先看看怎么做”或“给我一个流程建议”。

行为：

- 输入 request、constraints、known_context。
- 返回 suggested_nodes、suggested_edges、risk_notes、required_user_confirmations。
- 不创建可执行 workflow，不派发 child session。
- 不替 controller 生成最终 spec。

插件与大模型交互：

- 插件把内置 agent 能力和项目运行时约束整理成 tool result。
- controller 读取建议后自行生成或修改 `workflow_spec`，再向用户确认。
...
- report 中的 `next_suggestion` 只能作为上下文，不能作为 node agent 的调度命令。
...
-> controller intake
-> optional sp_status
-> optional sp_prepare suggestion
...
| 用户需求清楚，但流程形态不确定 | 请求插件建议，再生成 spec | `sp_prepare(mode="suggestion")` |
...
- `sp_prepare` 当前仍围绕固定 kind / prepare mode，需要支持 suggestion mode 和 `register_workflow`。
- transition 仍主要由固定 router/workflow 规则驱动，需要改为读取 `GeneratedWorkflowSpec`。
- 需要持久化 `workflow-spec.json`，并把 suggestion 和 registered spec 分开。
...
2. 扩展 `sp_prepare` / state schema，支持 suggestion 和 `workflow_spec` 注册。
3. 改 transition 与 fallback runtime，使动态 spec、`sp_report` 和 fallback summary 进入统一调度入口。
