...
Generate a workflow_spec when starting new work. Each node must have an agent, a scoped task, required context, expected output, report_contract, and transition condition. Include document contracts for workflow artifacts that the plugin must pass between nodes, such as run-local spec.md, plan.md, task_graph.json, task reports, or verification logs. Treat project docs such as docs/features, docs/bugfix, and docs/modules as workspace outputs unless the workflow explicitly imports them as source artifacts. Ask the user to confirm workflow execution when the workflow changes files, runs high-risk commands, or the project instructions require confirmation.
...
v5 需要把文档作为 workflow spec 的显式 contract，而不是让节点随手写文件。这里要区分三类文档：
...
- workspace output documents: node agent 写入项目工作区的交付文件，例如 `docs/features/*.md`、`docs/bugfix/*.md`、`docs/modules/*.md`。它们默认不是 node context，除非 workflow spec 明确声明插件要读取并作为 source artifact 传递。
...
node 通过 `consumes` 和 `produces` 引用 document id。`kind="workflow_artifact"` 的 `path` 相对 run 目录；`kind="workspace_output"` 的 `path` 相对项目工作区。插件校验这些引用，但不替 controller 决定哪些文档应该存在。
...
- implementer/reviewer/verifier node: 插件读取 canonical workflow artifacts 并内联到 node prompt；这些 node 不应自行在项目目录搜索 `spec.md` 或 `plan.md`。
- finisher/documentation node: 如果实现改变模块契约，负责在 finish 前更新 `docs/modules/*.md` 这类 workspace output，并通过 `sp_report` 记录路径和摘要。
...
- 本仓库规则要求的 `docs/features/`、`docs/bugfix/` 或 `docs/modules/` 是 workspace output。它们可以由节点生成和汇报，但不是默认的节点消费文档。
