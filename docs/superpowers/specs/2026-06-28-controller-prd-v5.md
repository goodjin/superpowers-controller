...
v5 的变化集中在一点：插件不再把内置 workflow kind 当作主决策来源，也不把“是否生成 workflow”作为核心分叉。controller 使用固定工具协议推进任务：先理解需求，再对每个任务调用 `sp_prepare` 生成 run-local task document 和用户确认内容；用户确认后，controller 调用 `sp_start`，用参数指定内置 workflow 代号、自定义 workflow 编排或单节点编排。是否让 `sp-designer` 参与，由 controller 根据任务风险决定，并体现在 `sp_start` 的启动参数中。

```text
controller intake 问清用户侧问题
controller 调用 sp_prepare 生成任务文档和确认内容
user confirms prepared task
controller 调用 sp_start，传入 built-in workflow id 或 workflow orchestration
plugin 校验并执行 start plan
node agent 执行单个 node 并调用 sp_report
agent report 可产生新任务或 workflow expansion
plugin 根据 prepared task + start plan + result + auto expansion policy 推进或反馈 controller
```
...
- 在用户确认 prepared task 后调用 `sp_start`。
- 根据任务风险判断是否需要 `sp-designer` 参与。
- 在 `sp_start` 参数中选择内置 workflow 代号、自定义 workflow 编排或单节点编排。
- 在 `sp_start` 参数中设置是否允许 report 自动生成后续节点。
- 调用 `sp_status` 获取当前事实和能力目录。
- 调用 `sp_prepare` 准备任务、持久化任务文档并生成用户确认内容。
- 调用 `sp_start` 按 prepared task 和 start plan 启动、恢复、重试或继续执行。
...
- 生成并持久化 prepared task document。
- 生成面向用户确认的 prepared task summary。
- 校验 `sp_start` 提交的内置 workflow 代号或 workflow orchestration。
- 持久化 `prepared-task.json`、`start-plan.json`、兼容性的 `workflow-spec.json`、`documents.json`、`state.json`、`events.jsonl`。
- 生成 node task packet。
- 创建、复用或恢复 child session。
- 读取 run-local workflow artifacts，并内联到 node prompt。
- 收集和校验 `sp_report`。
- 根据 `WorkflowState`、prepared task、start plan、node result 和 auto expansion policy 计算下一步。
- 在 start plan 允许时，校验并自动应用 agent-generated task graph 或 workflow expansion。
...
- 每个执行任务都必须先调用 `sp_prepare`；prepare 用于生成任务文档和给用户最终确认，不负责派发 agent。
- `sp_prepare` 之后，controller 向用户展示确认内容；用户确认后才调用 `sp_start`。
- controller 判断是否需要 `sp-designer` 参与；如果需要，在 `sp_start` 的 workflow 编排或内置 workflow 选择中体现。
- `sp_start` 可以接收内置 workflow 代号，也可以接收自定义 workflow 编排；自定义编排允许只有一个节点。
- 是否允许 agent report 自动生成后续节点，由 `sp_start.allow_auto_expansion` 或等价参数控制。
...
- 只做设计、只做计划或只跑指定节点时，controller 应在 `sp_start` 参数中关闭 auto expansion。
...
design or plan node -> terminal, with allow_auto_expansion=false
...
controller 可以裁剪、重排或省略节点，但需要保证传给 `sp_start` 的每个 node 有清晰输入、输出、报告契约和 transition 条件。plan 后的 implementation、review、verification 节点可以由 planner report 生成，并在 `allow_auto_expansion=true` 时自动进入 workflow。
...
- `plan-only`: designer/planner bounded run，默认 `allow_auto_expansion=false`。
...
| 只做 plan / design / review | 单节点或 small workflow 编排，并设置 `allow_auto_expansion=false` | bounded run |
...
每个要交给插件执行的任务都必须先经过 `sp_prepare`。`sp_prepare` 不派发 agent，不生成 workflow，不替 controller 判断是否需要 designer。它把 controller 已经澄清好的任务整理成 run-local prepared task，并返回给主控一份可以展示给用户确认的摘要。
...
- 写入 `prepared-task.json`、`request.md`、`task.md`、`documents.json`、`events.jsonl`。
...
定位：基于 prepared task 激活、恢复、重试或继续 execution。
...
type StartPlan =
  | BuiltInWorkflowStartPlan
  | CustomOrchestrationStartPlan

type BuiltInWorkflowStartPlan = {
  ...
  allow_auto_expansion: boolean
}

type CustomOrchestrationStartPlan = {
  kind: "orchestration"
...
- `start_prepared_task`: 校验 prepared task 已确认，校验 start plan，激活 execution，派发 initial runnable node。
...
- `expansion_ready`: 如果 `allow_auto_expansion=true` 且 expansion 由已允许的 node 产生并校验通过，自动应用并派发新 runnable node；不回到 controller。
...
| `passed` | 关闭 node，校验 artifacts/gates/report contract；如果包含 task graph 或 workflow expansion，则按 start plan 的 auto expansion policy 校验并追加。 | expansion 合法时自动派发新增 runnable node；否则按 start plan edge 推进。 |
...
- 取消后恢复必须读取当前 state、prepared task 和 start plan，不能回到固定 entrypoint。

## 6. Prepared Task And Start Plan

v5 的主协议拆成两层：

- `PreparedTask`: `sp_prepare` 产物，用于任务文档持久化和用户确认。
- `StartPlan`: `sp_start` 参数，用于声明启动方式和调度边界。
...
- `sp_prepare` 只生成 `PreparedTask`，不生成 `StartPlan`。
- `PreparedTask.status="confirmed"` 后才能 `sp_start(action="start_prepared_task")`。
- `sp_start` 必须携带 `StartPlan`。
- `StartPlan.kind="built_in_workflow"` 时，`workflow_id` 必须存在于 built-in workflow templates。
- `StartPlan.kind="orchestration"` 时，`orchestration.nodes` 可以只有一个 node。
- `nodes[].id` 唯一。
...
- `allow_auto_expansion=true` 时，允许被授权 node 的 `sp_report` 追加 task graph 或 workflow expansion，并在校验后继续执行。
- `allow_auto_expansion=false` 时，plugin 只运行 start plan 中已有 node；node report 中的 task graph 或 expansion 只作为 artifact 保存，不追加执行节点。
...
1. `sp_prepare` 写入 `prepared-task.json`、`request.md`、`task.md`、`documents.json`、prepared state 和事件日志。
2. 用户确认 prepared task 后，`sp_start(start_prepared_task)` 写入 `start-plan.json`，并把内置 workflow 或自定义 orchestration 规范化为兼容性的 `workflow-spec.json`。
3. `sp_start` 派发 node 前生成 `nodes/<node-id>/task.md`；有 `task_id` 时生成 `reports/<task-id>/task.md`。
4. designer/planner 等 node 通过 `sp_report.artifacts` 提交 `spec`、`plan`、`task_graph` 等 candidate。
5. plugin 把 candidate 写入 run 目录的 node record 或 output。
6. start plan 或 workflow orchestration 声明的 promotion 条件满足后，plugin materialize canonical artifact，例如 `spec.md`、`plan.md`、`task_graph.json`、`tasks.json`。
...
-> super-agent calls sp_prepare with clarified task brief
-> plugin writes prepared task documents and confirmation summary
-> super-agent asks user to confirm prepared task
-> user confirms
...
- 大模型理解需求、判断是否需要 designer、选择内置 workflow 或自定义编排、解释反馈。
- plugin 返回事实、agent catalog、built-in workflow templates、schema、prepared task、start plan 校验结果和下一步控制建议。
- 用户确认发生在主会话。
...
-> if task_graph/workflow_expansion exists and start plan allows auto expansion, validate and append nodes/edges/documents
-> compute transition from prepared task + start plan + workflow state
...
如果 `allow_auto_expansion=false`，plugin 不应用 report 中的新任务，只按 start plan 的 completion policy 结束或继续已有节点。transition 不明确、缺 artifact、fallback summary、权限不可用、校验失败或有高风险动作时，plugin 返回 controller decision 或 blocked。
...
1. 当前 `WorkflowState`。
2. prepared task 和 start plan。
3. 最新 node result: `sp_report` 或 fallback summary。
4. start plan 的 auto expansion policy 以及 report 中的 `task_graph` / `workflow_expansion`。
...
- agent-generated expansion 通过 schema、agent、edge、artifact 和 start plan auto expansion policy 校验。
...
- `allow_auto_expansion=false` 但 start plan 没有说明如何处理 report 中的新任务。
- agent-generated expansion 校验失败。
- report 与 spec condition 不匹配。
- report 缺少 required artifact。
- agent 不存在或权限不可用。
- prepared task 或 start plan 不完整。
...
- `sidebar_content`: prepared task、start plan、node graph、running/reported/fallback nodes、attention。
...
- `prepared-task.json` 必须在 `sp_prepare` 后落盘。
- `start-plan.json` 必须在 `sp_start(start_prepared_task)` 后落盘。
- `workflow-spec.json` 可作为兼容 alias 或规范化后的 workflow graph snapshot。
...
3. `sp_prepare` 写入 `prepared-task.json`、`request.md`、`task.md`、`documents.json` 和 events，不派发 child session。
4. `sp_prepare` 返回 `confirmation_summary` 和 `required_user_confirmations`，供 controller 在主会话向用户确认。
5. 用户确认 prepared task 后，controller 调用 `sp_start(action="start_prepared_task")`。
6. `sp_start` 可接收 `start_plan.kind="built_in_workflow"` 和内置 workflow 代号。
7. `sp_start` 可接收 `start_plan.kind="orchestration"` 和自定义 workflow 编排。
8. 自定义 workflow 编排允许只有一个 node。
9. controller 可在 `sp_start` 参数中决定是否让 `sp-designer` 参与。
10. controller 可在 `sp_start` 参数中设置 `allow_auto_expansion=true|false`。
11. `allow_auto_expansion=false` 时，plugin 不应用 planner 产出的 task graph 或 workflow expansion，只保存为 artifact。
12. `allow_auto_expansion=true` 且 expansion 合法时，plugin 自动把任务扩展成 execution/check/finish nodes 并继续派发。
13. `sp_report(status="progress")` 不派发下游。
14. `sp_report(status="failed")` 只有在 start plan 有明确 failure edge 时自动推进。
...
19. TUI 能展示 prepared task、start plan、动态 node graph、fallback、attention 和 progress digest。
...
|---|---|
| fixed workflow kind decides dispatch | prepared task plus controller-selected start plan decides dispatch |
| built-in feature/debug/review flows | built-in templates plus controller-provided start plan |
...
- start plan condition 第一版支持哪些 condition kind 的完整枚举。
