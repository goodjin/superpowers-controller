...
本版本是本轮实现基准。运行时代码、模块文档和测试应按本文约束对齐。public tool 行为不保留 v4 direct start 协议；实现内部如需读取旧状态用于迁移或诊断，必须先规范化为 v5 prepare/start/workflow-spec 状态，再进入运行时调度。

v5 的变化集中在一点：插件不再把内置 workflow kind 当作主决策来源，也不把“是否一次性生成完整 workflow”作为核心分叉。controller 使用固定工具协议推进任务：先理解需求，再对每个任务调用 `sp_prepare` 准备执行任务、生成既有 run-local artifacts、用户确认内容，以及必要的 prepare-stage workflow-spec；如果需要 `sp-designer` 参与，designer 也在 prepare 阶段作为头脑风暴/设计协作者运行。用户确认后，controller 调用 `sp_start`，用参数指定内置 workflow 代号、自定义 workflow 编排或单节点编排。plugin 必须把启动配置规范化为当前阶段的 `workflow-spec.json`，后续派发只依据 workflow-spec、state、report 和 policy 计算，不允许写死“某事件后固定派发某 agent”的隐式流程。

workflow-spec 可以分阶段形成：prepare 阶段可以只有 designer/confirmation 节点，planning 阶段可以只有 planner/approval 节点，execution 阶段可以由 planner report 或 controller orchestration 扩展出 implementation/check/finish 节点。设计阶段之前不要求已经存在完整执行 workflow，但任何实际派发的 node 都必须已经出现在当前 `workflow-spec.json` 中。

```text
controller intake 问清用户侧问题
controller 判断 prepare 阶段是否需要 sp-designer 参与
controller 调用 sp_prepare 生成既有任务 artifacts、确认内容和必要的 staged workflow-spec
user confirms prepared execution task
controller 调用 sp_start，传入 built-in workflow id 或 workflow orchestration
plugin 校验启动配置并规范化为当前阶段 workflow-spec
...
### 3.1 Controller: `superpowers-agent`
...
- 在 `sp_start` 参数中选择内置 workflow 代号、自定义 workflow 编排或单节点编排；不调用旧式 direct start payload。
...
- 把 controller 明确选择的 prepare/start/expansion 规范化为 staged `workflow-spec.json`。
...
- 收集和校验 `sp_report`。
- 根据 `WorkflowState`、`workflow-spec.json`、node result 和 auto expansion policy 计算下一步；dispatch 来源必须是 workflow-spec，而不是代码里的固定流程分支。
...
- 不绕过 `workflow-spec.json` 写死 event -> agent 的调度逻辑。
...
`superpowers-agent` prompt 应包含 Superpowers 工作理念、固定工具使用协议、内置 workflow templates 和常用 workflow 示例。templates 和 examples 只帮助 controller 选择或规划，不是固定流程，也不是 plugin 的智能建议。

### 4.1 Operating Principles

controller 应遵守：

- 每个新的 `superpowers-agent` 会话第一轮 assistant 回复必须先输出一句固定欢迎语：`欢迎使用superpowers主控插件，我将按superpowers工作流程完成您的任务。`
- 用户和项目指令优先。
...
- `sp_start` 不接收旧式 direct start payload；所有启动都必须引用 `prepared_task_id`、用户确认事实和 `StartConfig`。
- workflow-spec 可以分阶段生成和扩展；controller 不需要在 design 之前一次性生成完整 execution workflow。
- plugin 每次派发 node 前必须先确认该 node 已存在于当前 `workflow-spec.json`，并且 edge、artifact 和 policy 条件满足。
...
1. First-response rule: 每个新的 `superpowers-agent` 会话第一轮 assistant 回复必须先输出固定欢迎语，然后进入 intake 或 status 对齐。恢复、重试和状态查询如果发生在新的 `superpowers-agent` 会话第一轮，也必须输出；同一个会话后续轮次不重复输出。
...
7. Start rule: 用户确认后才调用 `sp_start(action="start_prepared_task")`。`sp_start` 只表达 v5 启动配置，可以是内置 workflow id，也可以是自定义 orchestration；orchestration 可以只有一个 node。旧式 direct `sp_start({ request, workflow, entrypoint, proposal })` 不属于 v5 public protocol。
...
当 prepare 阶段需要派发 designer 时，plugin 必须生成或更新 prepare-stage `workflow-spec.json`，其中显式包含 designer node、确认 gate、artifact contract 和恢复策略。prepare-stage spec 不要求覆盖后续 planning/execution；它只约束当前阶段可派发的节点。
...
- 需要 designer 参与时，先写入包含 designer node 的 prepare-stage `workflow-spec.json`，再派发 child session。
...
- `start_prepared_task`: 校验 prepare 结果、用户确认依据和启动配置，记录 `PrepareState.status="confirmed"`，把启动配置规范化为当前阶段 `workflow-spec.json`，激活 execution，派发 initial runnable node。
...
- direct start payload 无效；缺少 `prepared_task_id`、`confirmation` 或 `start_config` 时必须拒绝并返回可修正的 controller feedback。
...
- `expansion_ready`: 如果 auto expansion policy 允许，且 expansion 由已允许的 node 产生并校验通过，先把新增 nodes/edges/documents 写入 `workflow-spec.json`，再派发新 runnable node；不回到 controller。
...
| 启动预期流程 | `sp_start(start_prepared_task)` | 必须校验 `prepared_task_id`、用户确认依据和 start config，接收 built-in workflow id 或 custom orchestration，并规范化为 `workflow-spec.json`。 |
...
- `WorkflowSpec`: 存在 `workflow-spec.json` 中，是当前阶段可执行 graph 的事实源；它可以随 prepare、start、planner report 或 controller decision 分阶段生成和扩展。
...
- 用户确认前 `PrepareState.status` 保持 `"prepared"`；`sp_start(action="start_prepared_task")` 携带有效 `StartConfirmation` 后，plugin 才能把状态更新为 `"confirmed"` 并启动。
- `sp_start` 必须携带 `StartConfig`。
- public protocol 不支持绕过 `sp_prepare` 的 direct start；旧 payload 只能作为迁移输入被拒绝或转换为需要重新 prepare 的 controller feedback。
...
- built-in workflow id 只能实例化内置 template 得到 `workflow-spec.json`，不能在 runtime 中作为隐式 dispatch switch 使用。
- `nodes[].id` 唯一。
...
- 每次追加或替换 workflow graph 时，plugin 必须先持久化新的 `workflow-spec.json` 版本，再根据新版本计算 runnable nodes。
- 每个被派发的 node 必须存在于当前 `workflow-spec.json`；不存在时只能进入 controller decision 或 blocked，不能临时拼接 child prompt 直接运行。
...
- edge 只能引用存在的 node。
- transition 只能从 `workflow-spec.json` 的 edge、completion policy、fallback policy 和 controller decision edge 计算；不能通过代码里固定的 event kind 分支跳转。
...
1. `sp_prepare` 写入或更新 `request.md`、`documents.json`、`state.json` 和 `events.jsonl`；需要 designer 参与时，先生成 prepare-stage `workflow-spec.json`，再派发 designer，designer 输出进入 `spec.md` candidate 或 node record。
2. 用户确认 prepared execution task 后，`sp_start(start_prepared_task)` 把内置 workflow 或自定义 orchestration 规范化为当前 stage 的 `workflow-spec.json`，并在 `state.json` 中记录确认依据、启动配置和 auto expansion policy。
...
8. planner report 或 controller decision 产生合法 expansion 时，plugin 先更新 `workflow-spec.json`，再派发 expansion 中的 runnable nodes。
9. node 不自行搜索 run 目录之外的 `spec.md` 或 `plan.md`。
10. `sp_report(status="progress")` 只能产生 candidate/progress，不解锁下游。
...
```text
user request
-> superpowers-agent intake
-> sp_status
-> optional sp_status(include_capabilities=true)
-> superpowers-agent calls sp_prepare with clarified task brief and optional design participation
-> plugin writes existing run-local artifacts, staged workflow-spec when needed, and confirmation summary
-> superpowers-agent asks user to confirm prepared execution task
-> user confirms
-> superpowers-agent calls sp_start with confirmation and built-in workflow id or custom orchestration
```

分工：

- 大模型理解需求、判断 prepare 阶段是否需要 designer、选择内置 workflow 或自定义编排、解释反馈。
- plugin 返回事实、agent catalog、built-in workflow templates、schema、prepare state、workflow-spec、启动配置校验结果和下一步控制建议。
- 用户确认发生在主会话。
...
-> if task_graph/workflow_expansion exists and workflow policy allows auto expansion, validate and append nodes/edges/documents into workflow-spec
-> compute transition from state + workflow-spec + node result
-> dispatch next node or return controller_feedback
```

如果 `workflow_expansion` 合法，plugin 自动追加新 nodes/edges/documents 到 `workflow-spec.json`，并重新计算 runnable nodes。
...
planner passed 后的 implementation、acceptance、verification、code-review、finish 节点应走这条路径继续执行，不默认回到 controller；但这些节点在派发前必须已经由 expansion 或 built-in template 写入 `workflow-spec.json`。
...
-> superpowers-agent asks user
-> user answers
-> superpowers-agent calls sp_start(resume_input)
...
`workflow-spec.json` 是调度事实源。runtime 可以识别内置 workflow template、auto expansion policy 和 report event kind，但这些只能用于生成、校验或扩展 workflow-spec；不能绕过 workflow-spec 直接派发下一个 agent。
...
- 匹配的 edge 和目标 node 存在于当前 `workflow-spec.json`。
- 下一个 node agent 存在。
...
- 多条 edge 同时匹配且优先级不明确。
- report event kind 看似能推出下一个阶段，但 `workflow-spec.json` 中没有对应 edge 或 node。
...
- prepare 阶段派发 designer 前，prepare-stage `workflow-spec.json` 必须已经落盘。
- `sp_start(start_prepared_task)` 后，用户确认依据、启动配置、内置 workflow 展开结果、自定义 orchestration 和 auto expansion policy 必须写入 `state.json` 与 `workflow-spec.json`。
- `workflow-spec.json` 是规范化后的 workflow graph snapshot，也是 runtime dispatch 的事实源。
...
- 恢复时如果 durable state 与 `workflow-spec.json` 不一致，不能根据 workflow_id 或 event kind 补派节点；必须进入 controller decision 或要求重新 prepare/start。
...
6. `sp_start(start_prepared_task)` 必须携带 `prepared_task_id`、`confirmation` 和 `start_config`；旧式 direct start payload 被拒绝或返回要求重新 prepare 的 controller feedback。
7. `sp_start` 可接收 `start_config.kind="built_in_workflow"` 和内置 workflow 代号。
8. `sp_start` 可接收 `start_config.kind="orchestration"` 和自定义 workflow 编排。
9. 自定义 workflow 编排允许只有一个 node。
10. controller 可在 `sp_prepare` 参数中决定是否让 `sp-designer` 参与 prepare-phase brainstorming/design。
11. prepare-phase designer 派发前，`workflow-spec.json` 中必须已经存在 designer node 和相关 gate。
12. 设计阶段之前不要求完整 execution workflow；workflow-spec 可以在 prepare、planning、execution 阶段分步生成和扩展。
13. 每个被派发的 node 都必须存在于当前 `workflow-spec.json`。
14. 内置 workflow id 只能实例化 workflow-spec，不能作为 runtime hardcoded dispatch switch。
15. `design-only`、`plan-only`、`review-only` 等 `*-only` 内置 workflow 默认不自动扩展。
16. auto expansion policy 禁止扩展时，plugin 不应用 planner 产出的 task graph 或 workflow expansion，只保存为 artifact。
17. auto expansion policy 允许且 expansion 合法时，plugin 先更新 `workflow-spec.json`，再自动把任务扩展成 execution/check/finish nodes 并继续派发。
18. `sp_report(status="progress")` 不派发下游。
19. `sp_report(status="failed")` 只有在 workflow 有明确 failure edge 时自动推进。
20. `sp_report(status="needs_user")` 写入 pending question，并通知 parent controller session。
21. `sp_start(resume_input)` 恢复原 child session，不创建新 node。
22. child session 无 terminal report 时生成 fallback summary，并进入 controller decision。
23. `spec.md`、`plan.md` 等 workflow artifacts 只从 run 目录读取并内联给 node agent。
24. TUI 能展示 prepared execution task、workflow-spec、动态 node graph、fallback、attention 和 progress digest。
25. startup recovery 不把 durable running 当成 live running。
26. public tool surface 不新增工具，仍是 `sp_status`、`sp_prepare`、`sp_start`、`sp_cancel`、`sp_report`。
27. controller 首次进入 Superpowers 工作流时输出固定欢迎语。
28. 详细执行过程使用 OpenCode 原生 child session 展示；主会话只显示确认、摘要、入口、attention 和按需 progress digest。

## 14. Migration Notes From V4

| V4 concept | V5 replacement |
|---|---|
| fixed workflow kind decides dispatch | confirmed prepare state plus controller-selected start config creates workflow-spec; workflow-spec decides dispatch |
| direct `sp_start` with request/workflow/entrypoint/proposal | `sp_prepare` -> user confirmation -> `sp_start(start_prepared_task)` with `StartConfirmation` and `StartConfig` |
| built-in feature/debug/review flows | built-in templates instantiate workflow-spec plus controller-provided start config |
...
v5 不移除现有 agents 或 public tools。它移除的是旧 direct start 公共协议，以及 runtime 用固定 workflow kind 或 event kind 隐式选择下一个 agent 的行为。旧状态可用于迁移、审计和错误提示，但进入可执行 runtime 前必须落到 v5 prepare state、start config 和 `workflow-spec.json`。
...
- staged `workflow-spec.json` 的版本号、patch 记录和 TUI diff 展示格式。
- `StartConfirmation.user_message` 是否需要保存完整用户原文，还是只保存 session/message 引用。
