...
- `src/session/parent-progress-notifier.ts`：读取同一份 workflow state 和 child progress，向 parent session 每 30 秒提交一条简短可见进度。
...
有一个例外是 parent session 周期性进度提示。它不是 `ProgressUpdate` toast，而是由 `parent-progress-notifier` 每 30 秒向 `parent_session_id` 提交一条短 prompt，让 `super-agent` 在主会话里直接告诉用户当前 child session 或 workflow 的进展。该 prompt 使用现有 progress panel 摘要，且明确要求只输出进度、不调用工具、不推进 workflow。

这条 parent prompt 只在 workflow `status=running` 且存在 running child node 时发送，并在 workflow 进入等待用户、等待裁决、阻塞、失败、取消、完成，或没有 running child node 时停止。它服务“等待时不要静默”的可见性目标，不改变 workflow state。
...
- `app_bottom`：主会话底部常驻 surface，承载整体 workflow 状态，例如 workflow/status/current phase、任务完成数、运行中 session 数，并追加最近一个 child session 的可读活动摘要和相对更新时间。OpenTUI 的 `app_bottom` slot 不提供 `session_id`，因此这里按全局 active/unfinished workflow 渲染；只要当前项目或 fallback project 中存在未结束 workflow，就显示底部状态。
