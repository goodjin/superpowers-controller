...
- **liveness monitor**：默认每 15s 检查一次，若 `running` node 的 progress（或 `started_at`）超过 5 分钟无更新，则标 `interrupted` 并写入 `liveness_timeout` 事件。TUI `stalled` 阈值仍为 30s，只负责早提示，不负责 state 降级。
