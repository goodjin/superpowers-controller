...
- `src/tui/progress-panel.ts`：把 workflow state、节点进展和 live session status 整理成 TUI 面板 view-model。侧栏 child 列表经 `visibleNodeRunsForDisplay()` 过滤：同 `task_id+phase`（无 task 时按 `phase`）只保留最新一条，避免 retry 后旧 `interrupted`/`canceled` 占行；磁盘 `node_runs` 不删。终态（`passed`/`canceled`）且无 `node_runs` 时侧栏不展示 workflow 进度块，改走主控 / host session focus；空节点文案区分「waiting for node dispatch」与「workflow canceled/finished · no child sessions」。
...
- `src/runtime/startup-recovery-gate.ts`：短 CLI 跳过写盘启动恢复；TUI 侧栏对「interrupted 但仍 busy」的节点做写回自愈（见 state 模块）。
...
- **liveness monitor**：默认每 15s 检查一次，若 `running` node 的 progress（或 `started_at`）超过 5 分钟无更新，则标 `interrupted` 并写入 `liveness_timeout` 事件。例外：从后往前看，最近工具生命周期仍是 `tool_running` / `tool_pending`（尚未 `tool_completed` / `tool_error`，且其后没有 `session_idle` / `session_error`）时，在最长 45 分钟 grace 内不超时。TUI `stalled` 阈值仍为 30s，只负责早提示，不负责 state 降级。

进展以 JSONL 追加到：

```text
.superpowers/runs/<run-id>/nodes/<node-id>/progress.jsonl
```
...
`sidebar_content` 的列表结构向 OpenCode 原生 TodoWrite 的可扫读形态靠拢，但数据源仍然是 Superpowers workflow state。短标题后直接列已创建的 child（active 优先），再列 `planned` 未派发任务。同 phase / task+phase 的多次 attempt 只显示最新一条。child 行只保留 shortcut、agent、task、状态与一条活动摘要；完整 session id 与 node id 不进侧栏正文。
