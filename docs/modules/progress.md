...
这条按需查询通道不能替代 `app_bottom` 或 `sidebar_content`。TUI 仍是常驻可见 surface；dispatch/resume 后 orchestrator 会切到 child session，让用户直接观察 live transcript。等待用户、等待批准、阻塞、失败等关键事件仍由 controller 在主会话里显式说明。
...
- `app_bottom`：主会话底部常驻 surface（档 B Live Activity 条），模拟原生 task 卡片条但不写入 parent transcript。第一行是 workflow 摘要（status/phase、任务完成数、active child 数）；下面列出最多 5 行 child sessions，每行含 `>` 聚焦标记、`[⌘n]` 快捷键、agent/task、状态与 progress 摘要。running child 的摘要优先使用 TUI live messages 中的 `↳ Tool title` 格式，其次才是 `progress.jsonl`。若有 planned task 再追加简短 planned 列表；末行显示 `nav: ⌘1..⌘n, ⌘[/⌘]`。订阅 `api.event.on("message.part.updated" | "session.status" | ...)` 事件触发刷新，1s 轮询作兜底。
- `sidebar_content`：右侧栏主体，是 workflow 会话运行信息的主展示区域。OpenCode host 会传入当前 `session_id`；插件优先在该 session 属于某个 workflow 的 parent session 或 child node session 时展示 workflow summary、total sessions、running sessions、attention、shortcut hints、运行中 child session 列表（同样优先 live `↳ Tool title` 摘要）、计划中/待运行 task 列表、最近活动详情，以及 waiting_user 的问题正文和候选选项。事件驱动刷新策略与 `app_bottom` 相同。
