...
- `src/session/parent-progress-notifier.ts`：读取同一份 workflow state 和 child progress，在 parent-led 并行阶段通过 progress surface 发布简短可见进度；它不向 parent session 写 prompt。
...
- `parent_progress`：parent-led child workflow 的周期性进度 surface；不进入 OpenCode 会话上下文。
- `parent_progress_failed`：parent progress surface 发布失败，workflow state 不因此改变。
...
parent session 周期性进度也是普通 progress。`parent-progress-notifier` 每 10 秒读取 workflow/progress，复用 progress panel 摘要生成 `parent_progress` surface，并通过 `ctx.client.tui.showToast()` 或 app log 发布。它不调用 `session.prompt()`，因此不会把重复进度写入 parent session 历史，也不会撑大后续模型上下文。notifier 对稳定进度 key 去重：状态没有语义变化时，不会因为 `observed_age` 变化而反复发布。

这条 parent progress 只在 workflow `status=running` 且存在非 design/plan 的 running child node 时启用，并在 workflow 进入等待用户、等待裁决、阻塞、失败、取消、完成，或没有 parent-led running child node 时停止。design/plan child 是前台串行阶段，用户直接观看 child session，不通过 parent progress 刷屏。parent progress 服务“并行等待时不要静默”的可见性目标，不改变 workflow state。
...
这条按需查询通道不能替代 `app_bottom`、`sidebar_content` 或 parent progress notifier。TUI 仍是常驻可见 surface；parent notifier 只发短摘要 surface，避免用户长时间看不到任何动静，同时不污染主会话上下文。等待用户、等待批准、阻塞、失败等关键事件仍由 controller 在主会话里显式说明。
...
- `sidebar_content`：右侧栏主体，是 workflow 会话运行信息的主展示区域。OpenCode host 会传入当前 `session_id`；插件优先在该 session 属于某个 workflow 的 parent session 或 child node session 时展示 workflow summary、total sessions、running sessions、attention、shortcut hints、运行中 child session 列表、计划中/待运行 task 列表、最近活动详情，以及 waiting_user 的问题正文和候选选项。如果当前存在 foreground/running child，sidebar 还会读取 `api.state.session.messages(childID)`、`status(childID)`、`permission(childID)` 和 `question(childID)`，追加该 child 的 live transcript 摘要和等待项数量。waiting permission 的 running child 会优先在这里变得可见，避免权限确认只停留在 child session 内部。如果没有 session 匹配，允许 fallback 到最新未结束 workflow。不能依赖 host 一定传 `agent` 字段；实际 host 可能只传 `session_id`。它不收集用户答案，也不替换 host 原生摘要。
- `sidebar_footer`：右侧栏底部降级 surface，承载整体 workflow 状态。和 `sidebar_content` 一样优先按 `session_id` 绑定，找不到匹配时 fallback 到最新未结束 workflow。
- `session_prompt`：当当前 session 是 foreground/running child，或当前 session 是 `parent_session_id` 且存在 foreground/running child 时注册有效内容。它使用 OpenCode TUI 的 `api.ui.Prompt({ sessionID: childID })` 保持底部输入区域并把提交目标绑定到当前 foreground child。没有 foreground child 时返回空，不影响普通 prompt。
...
`sidebar_content` 的列表结构向 OpenCode 原生 TodoWrite 的可扫读形态靠拢，但数据源仍然是 Superpowers workflow state。它先给出 workflow summary 和 session counts，再列 `child sessions`，展示已经创建的 node session，并把 running / waiting permission / stalled / needs_user 等 active sessions 排在前面；随后列 `planned sessions`，从 `task_graph.tasks` 和 `node_runs` 推导 pending/blocked/dispatch_failed/interrupted 等未完成任务。每行包含 agent、task id、状态、标题、session id 缩写和可用 shortcut hints；已有 child session 的行还会显示 latest progress summary 和 detail。

当 workflow 进入 `waiting_user` 时，`sidebar_content` 会显示 pending question 的来源节点、问题正文、最多三个候选选项，以及最近 child 活动。用户回答由 runtime 选择的 foreground target 处理：design/plan foreground child 通过 parent session 的 `session_prompt` slot 提交到 child；其他阶段回 parent controller session。TUI slot 不承担独立选择框或自由输入框职责。
