...
- `start_config` 可选：省略时按 prepared run 的 `workflow` 生成内置执行图；传入时覆盖模板或自定义 orchestration。runtime dispatch 以落盘后的 `workflow_spec` 为准。
...
candidate 不会绕过用户确认直接启动正式执行。v5 主路径由 `sp_prepare` 生成 confirmation summary 和 prepare-stage `workflow-spec.json`，用户确认后由 `sp_start(action="start_prepared_task", confirmation)` 激活（`start_config` 可选）；启动时 runtime 把内置模板或自定义 orchestration 写成执行期 `workflow-spec.json`，再生成或更新 canonical artifacts、task graph 和 dispatch state。
...
Transition 在 `code-review` passed 后回到 workflow-spec 计算 runnable nodes（跨 task 依赖已指向前序 `…-code-review`）；如果 finisher 的 depends_on 均已满足则派发 finish，否则继续后续 implement。

`task_graph` 只负责生成/更新完整 `workflow-spec`；调度与完成判定按节点图执行，不再用 task-level 作为第二套门禁。详见 `docs/features/2026-07-18-spec-only-dispatch.md`。

后续如果要支持每个 task 自定义裁剪 checks，应把裁剪结果写进 workflow-spec 节点，而不是在 runtime 另开一套 task 完成模型。
