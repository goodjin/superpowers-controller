...
这个 parent prompt 是补充通道，不是当前 running assistant turn 的文本插入 API。OpenCode `session.promptAsync` 会调度一个新的 parent session turn；如果 parent session 当前仍在运行，主会话消息流不保证在 30 秒时立刻出现文本。等待时的即时可见性由 TUI resident surfaces 承担，尤其是 `app_bottom` 和 `sidebar_content`。
