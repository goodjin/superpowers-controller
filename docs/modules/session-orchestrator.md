...
10. 若 child session 在仍为 `running` 时变为 `session.idle`（或随后被 liveness / `session.error` 闭合）且从未 `sp_report`，plugin 会采集最后助手文字与产出路径，写入 `nodes/*/silent-exit.*`，进入 `waiting_controller_decision`，再 `returnToParent` 切回主控并 toast，随后 `notifyParent` 把证据交给主控；不会自动推进下一节点。
