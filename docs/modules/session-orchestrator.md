...
- `src/session/adapter.ts`：封装 OpenCode SDK 的 `session.create`、`session.prompt`、`tui.selectSession` / `tui.session.select`、`tui.showToast` 和 `app.log` fallback。
...
8. orchestrator 根据 phase 选择前台 session：design/plan 选择 child session；implement/acceptance/verification/code-review 等 parent-led 阶段选择 parent session。
9. dispatch 方法返回。
...
design 和 plan 是串行前台阶段。创建或复用这类 child session 后，orchestrator 会请求 OpenCode TUI 选择该 child session。候选 design 或 plan 需要批准、修改或取消时，等待确认 prompt 会继续投到当前 child session；TUI 插件通过 `session_prompt` slot 显式绑定 `api.ui.Prompt({ sessionID: childID })`，保证用户在 child route 底部输入时提交目标仍是该 child session，由 child 调用 `sp_start(approve_design)` 或 `sp_start(approve_plan)` 推进 workflow。
...
这个 parent prompt 是补充通道，不是当前 running assistant turn 的文本插入 API。OpenCode `session.promptAsync` 会调度一个新的 parent session turn；如果 parent session 当前仍在运行，主会话消息流不保证在 10 秒时立刻出现文本。等待时的即时可见性由 TUI resident surfaces 承担，尤其是 `app_bottom` 和 `sidebar_content`。

notifier 只在 workflow `status=running` 且存在非 design/plan 的 `running` child node 时发送。workflow finished、canceled、blocked、failed、waiting_user、waiting_user_decision、waiting_controller_decision，或没有 parent-led running child node 时，下一次 tick 会清理定时器且不再发消息。
