...
- `src/session/orchestrator.ts`：根据 create/reuse dispatch 调用 adapter，封装 parent notification、child resume，并在 dispatch/resume 后请求 TUI 选择 child session。
...
7. orchestrator 请求 OpenCode TUI 选择刚创建、复用或 resume 的 child session。host 不支持 session focus 时，dispatch/resume 仍继续，TUI/sidebar 负责提供切换提示。
