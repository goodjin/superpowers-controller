...
7. orchestrator **不**自动切到 child；用户默认留在主控，需要细节时主动进入 child。host 不支持 session focus 时也不阻塞 dispatch。
8. dispatch/resume 方法返回。
9. child session 正常结束时通过 `sp_report` 回到 state/transition 层。
10. 若 child session 在仍为 `running` 时变为 `session.idle`（或随后被 liveness / `session.error` 闭合）且从未 `sp_report`，plugin 会采集最后助手文字与产出路径，写入 `nodes/*/silent-exit.*`，进入 `waiting_controller_decision`，再由 orchestrator `notifyParent` 把证据交给主控；不会自动推进下一节点。
...
当 child session 通过 `sp_report(status="needs_user")` 请求用户输入时，report handler 会先写入 state，再调度一条等待用户输入的 prompt，并 `selectSession` 到**主控会话**（`parent_session_id` + `superpowers-agent`）。design/plan 澄清/审批同样投主控。通知后台提交；插件不在 TUI 里额外做选择题面板。
...
用户通过主会话回答后，`sp_start(run_id, resume_input)` 会调用 store 消费 `pending_question`，再通过 `orchestrator.resumeNode()` 把 resume prompt 发回原 `node_runs[].session_id`。resume 后**不**自动切 child。这不是新的 dispatch decision，不创建新 child session，也不改变 task graph。`sp_start` 在 resume prompt 调度后返回，不等待该 child session 完成。
...
- dispatch / resume / permission **不**自动切到 child
- `needs_user` 始终通知主控
- 主控页底部输入保持给主控；只有用户主动进入子会话页时，输入才绑到该子会话
