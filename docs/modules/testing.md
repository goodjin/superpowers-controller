...
- `test/tui-plugin.test.ts` 覆盖 `superpowers-progress` route、parent workflow session command、resident progress slot 名单、foreground child prompt binding、`app_bottom` no-props 渲染 active workflow、`app_bottom` refresh 重读 progress、`sidebar_content` 作为 workflow 会话运行信息主展示区域、`session_prompt_right` / `home_prompt` / `home_prompt_right` 不注册、`sidebar_content` no-props 全局进度/主会话运行列表、无 agent 的新主控会话 fallback，以及 parent/child/no-props/unrelated session 下的 compact progress 行。
