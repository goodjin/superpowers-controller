...
- `sp_start`：只在用户确认 prepared task 后启动。v5 主路径是 `action="start_prepared_task"`、`prepared_task_id`、`confirmation: StartConfirmation` 和 `start_config: StartConfig`。`StartConfig` 可以引用内置 workflow template，也可以提供单节点或多节点 orchestration；runtime 会以 `workflow-spec.json` 为 dispatch 来源。对异常态，`sp_start(start_action="resolve_controller_decision", controller_decision=...)` 消费 `allowed_controller_decisions` 中的裁决。`sp_start` 派发或恢复 child prompt 后返回，不等待 child session 完整跑完。
