...
Superpowers Controller 是通过 Agent 使用 Superpowers 框架的 OpenCode 插件。安装后默认入口为 `superpowers-agent`。
...
安装器会把 OpenCode 的 `default_agent` 设为 `superpowers-agent`。之后直接启动即可：
...
安装后可用 `bunx superpowers-controller doctor` 检查环境。

## 我们想解决什么

1. **一次装好，自动用技能**  
   一次性安装并配置好 Superpowers 框架；Agent 会主动提醒模型使用相应技能，无需手动触发。

2. **子会话隔离，不污染主控**  
   插件自动创建子会话执行各个技能；各会话与主控会话天然隔离，避免上下文被技能细节污染。

3. **任务可持久化，重启可续跑**  
   任务相关文档与工作流都会落盘；系统重启后可从断点恢复执行，不丢任务。

4. **按任务选工作流，也可自定义**  
   插件会按任务类型提醒模型采用不同工作流，也允许模型自定义工作流。

大模型负责理解任务、拆分任务和完成节点产出；插件负责维护状态、调度节点、记录结果，减少长上下文污染和流程跑偏。
...
每次 prepare、start、report、gate、节点结果和异常状态都会落到项目本地文件里，方便中断后恢复，也方便回头跟踪任务为什么走到某一步、哪个节点提交了什么证据。
